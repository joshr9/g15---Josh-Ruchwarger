...
function shotChart(player) {
  $.post($url_shotChart + '?api_key=' + $api_key + '&player_id=' + JSON.parse(player)[0].player_id, function(d){
    console.log(d);
...
function getStats(player) {
  ...
  $.post($url_playerStats + '?api_key=' + $api_key +  '&player_id=' + JSON.parse(player)[0].player_id + '&season=2015', function(d){
